...
void EEPROM_Write(unsigned char addr, unsigned char data) {
    while(EECON1bits.WR) __asm("clrwdt");
...
unsigned char EEPROM_Read(unsigned char addr) {
    while(EECON1bits.WR) __asm("clrwdt"); EEADR=addr; EECON1bits.EEPGD=0; EECON1bits.RD=1; return EEDATA;
}

void Lcd_Port(char a){ if(a&1)RD4=1;else RD4=0; if(a&2)RD5=1;else RD5=0; if(a&4)RD6=1;else RD6=0; if(a&8)RD7=1;else RD7=0; }
void Lcd_Cmd(char a){ RD2=0;Lcd_Port(a);RD3=1;_delay((unsigned long)((4)*(20000000/4000.0)));RD3=0; }
...
void Lcd_Write_Char(char a){ char t,y; t=a&0x0F; y=a&0xF0; RD2=1; Lcd_Port(y>>4); RD3=1; _delay((unsigned long)((40)*(20000000/4000000.0))); RD3=0; Lcd_Port(t); RD3=1; _delay((unsigned long)((40)*(20000000/4000000.0))); RD3=0; }
void Lcd_Write_String(char *a){ int i; for(i=0;a[i]!='\0';i++)Lcd_Write_Char(a[i]); }
void Lcd_Init(){ Lcd_Port(0);_delay((unsigned long)((20)*(20000000/4000.0)));Lcd_Cmd(3);_delay((unsigned long)((5)*(20000000/4000.0)));Lcd_Cmd(3);_delay((unsigned long)((11)*(20000000/4000.0)));Lcd_Cmd(3);Lcd_Cmd(2);Lcd_Cmd(2);Lcd_Cmd(8);Lcd_Cmd(0);Lcd_Cmd(0x0C);Lcd_Cmd(0);Lcd_Cmd(0x06); }

void UART_Init(long baud){ SPBRG=(unsigned char)((20000000/(16*baud))-1); TXSTAbits.TXEN=1; TXSTAbits.BRGH=1; RCSTAbits.SPEN=1; RCSTAbits.CREN=1; }
void UART_Write(char d){ while(!TXSTAbits.TRMT) __asm("clrwdt"); TXREG=d; }
void UART_Write_String(char *s){ while(*s) UART_Write(*s++); }
char UART_Read(){ while(!PIR1bits.RCIF) __asm("clrwdt"); if(RCSTAbits.OERR){RCSTAbits.CREN=0;RCSTAbits.CREN=1;} return RCREG; }
unsigned char UART_Data_Ready(){ return PIR1bits.RCIF; }

void ADC_Init(){ ADCON0=0x41; ADCON1=0x8E; }
unsigned int ADC_Read(){ _delay((unsigned long)((20)*(20000000/4000000.0))); GO_nDONE=1; while(GO_nDONE) __asm("clrwdt"); return((unsigned int)ADRESH<<8)+ADRESL; }
...
void main(void) {
    ...
    while(1) {
        ...
        if(PORTBbits.RB1 == 0) {
            ...
            while(PORTBbits.RB1==0) __asm("clrwdt");
            menu++;
            if(menu>3) menu=1;
        ...
        if(PORTBbits.RB2 == 0 && !abortar) {

            if(menu==1) {
                Lcd_Set_Cursor(3,1);
                Lcd_Write_String("LISTENING...");
                while(PORTBbits.RB2==0) __asm("clrwdt");
                while(!abortar) {
                    __asm("clrwdt");
                    ...
                    if(UART_Data_Ready()) {
                        ...
                        if(c=='u' || c=='l') {
                            ...
                            _delay((unsigned long)((1500)*(20000000/4000.0))); break;
                        }
                    }
                }
            }
            ...
            if(menu==3) {
                ...
                while(!abortar && PORTBbits.RB1==1) {
                    ...
                    Lcd_Write_String(s);
                    if(PORTBbits.RB4 == 0) {
                        while(PORTBbits.RB4 == 0 && !abortar) {
                            if(intes <= 79) {
                                Lcd_Set_Cursor(4,1);
                                Lcd_Write_String(">>> SENDING ATTACK <<<");
                                UART_Write('B');
                            }
                            else if(intes >= 80 && intes < 95){
                                Lcd_Set_Cursor(4,1);
                            ...
                            _delay((unsigned long)((50)*(20000000/4000.0))); __asm("clrwdt");
                        }
                        Lcd_Set_Cursor(4,1);
                        Lcd_Write_String("                       ");
                    }
